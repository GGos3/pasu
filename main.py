...
score1 = pg.image.load('skin/score0.png')  # 숫자 이미지
score2 = pg.image.load('skin/score0.png')  # 숫자 이미지
score3 = pg.image.load('skin/score0.png')  # 숫자 이미지
score4 = pg.image.load('skin/score0.png')  # 숫자 이미지
score5 = pg.image.load('skin/score0.png')  # 숫자 이미지
score6 = pg.image.load('skin/score0.png')  # 숫자 이미지
score7 = pg.image.load('skin/score0.png')  # 숫자 이미지
score8 = pg.image.load('skin/score0.png')  # 숫자 이미지
score9 = pg.image.load('skin/score0.png')  # 숫자 이미지
...
while running:
    ...
    for event in pg.event.get():  # 파이게임 이벤트
        if event.type == pg.QUIT:
            ...
            if not isPlay:
                isPlay = True
                pg.mixer.music.play()
        #elif event.tpye == pg.KEYDOWN and event.key == pg.K_LCTRL and pg.K_r:
    ...
    if not isPlay:  # 플레이중이 아닐 때 임시처리 (추후에 퍼즈 기능으로 사용)
        ...
        if exposure_time > 0:
            ...
            if last_hit == hit300g:
                screen.blit(last_hit, (185, 500))
            if last_hit == hit300:
                screen.blit(last_hit, (185, 500))
            if last_hit == hit200:
                screen.blit(last_hit, (230, 500))
            if last_hit == hit100:
                screen.blit(last_hit, (235, 500))
            if last_hit == hit50:
                screen.blit(last_hit, (235, 500))
            if last_hit == hit0:
                screen.blit(last_hit, (235, 500))
